...
public class transactionsDriver {
	...
	public static void main(String[] args) throws Exception {
		...
		if (success)
		{
			...
			// TODO: specify input and output DIRECTORIES (not files)
			FileInputFormat.setInputPaths(job2, new Path("tempout1"));
			FileOutputFormat.setOutputPath(job2, new Path("tempout2"));
			
			success = job2.waitForCompletion(true);
			
			if(success)
			{
				Job job3 = Job.getInstance(conf, "TOP N");
				...
				// TODO: specify input and output DIRECTORIES (not files)
				FileInputFormat.setInputPaths(job3, new Path("tempout2"));
